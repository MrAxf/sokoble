...
export default function Box({ x, y, inButton }: BoxProps) {
  ...
  return (
    <motion.div
      className={`absolute aspect-square rounded-md transition ${ inButton ? 'bg-box-success-primary' : 'bg-box-primary' }`}
    ...
    >
      <div className={`w-1/2 transition rounded-sm ${ inButton ? 'bg-box-success-secondary' : 'bg-box-secondary'} aspect-square m-[25%]`}></div>
    </motion.div>
  )
}
